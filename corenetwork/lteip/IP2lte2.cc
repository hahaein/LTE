...
#include "corenetwork/lteip/IP2lte2.h"
...
Define_Module(IP2lte2);

void IP2lte2::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        else if (nodeType_ == ENODEB)
        {
            ...
            LteDeployer2 * deployer = check_and_cast<LteDeployer2*>(enodeb->getSubmodule("deployer"));
...
void IP2lte2::handleMessage(cMessage *msg)
{
    if( nodeType_ == ENODEB )
    {
...
void IP2lte2::setNodeType(std::string s)
{
...
void IP2lte2::fromIpUe(IPv4Datagram * datagram)
{
...
void IP2lte2::toIpUe(IPv4Datagram *datagram)
{
    EV << "IP2lte::toIpUe - message from stack: send to IP layer" << endl;
    send(datagram,ipGateOut_);
}

void IP2lte2::fromIpEnb(IPv4Datagram * datagram)
{
...
void IP2lte2::toIpEnb(cMessage * msg)
{
    EV << "IP2lte::toIpEnb - message from stack: send to IP layer" << endl;
    send(msg,ipGateOut_);
}

void IP2lte2::toStackEnb(IPv4Datagram* datagram)
{
    // obtain the encapsulated transport packet
    cPacket * transportPacket = datagram->getEncapsulatedPacket();
...
void IP2lte2::printControlInfo(FlowControlInfo* ci)
{
...
void IP2lte2::registerInterface()
{
...
void IP2lte2::registerMulticastGroups()
{
...
void IP2lte2::triggerHandoverSource(MacNodeId ueId, MacNodeId targetEnb)
{
...
void IP2lte2::triggerHandoverTarget(MacNodeId ueId, MacNodeId sourceEnb)
{
...
void IP2lte2::sendTunneledPacketOnHandover(IPv4Datagram* datagram, MacNodeId targetEnb)
{
...
void IP2lte2::receiveTunneledPacketOnHandover(IPv4Datagram* datagram, MacNodeId sourceEnb)
{
...
void IP2lte2::signalHandoverCompleteSource(MacNodeId ueId, MacNodeId targetEnb)
{
    EV << NOW << " IP2lte::signalHandoverCompleteSource - handover of UE " << ueId << " to eNB " << targetEnb << " completed!" << endl;
    hoForwarding_.erase(ueId);
}

void IP2lte2::signalHandoverCompleteTarget(MacNodeId ueId, MacNodeId sourceEnb)
{
