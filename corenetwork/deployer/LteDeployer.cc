...
{
   // mobility=check_and_cast<StationaryMobility*>(getParentModule()->getSubmodule("mobility"));
    numRelays_ = 0;
    relayDistance_ = 0;
    nodeX_ = 0;
    nodeY_ = 0;
...
//
  ...
  // Coord position;
    //mobilitydmodule=check_and_cast<StationaryMobility*>(getParentModule()->getSubmodule("mobility"));

  // position=mobilitydmodule->getCurrentPosition();
   // nodeX_=position.x;
   // nodeY_ = position.y;
   // nodeZ_ = position.z;
...
void LteDeployer::initialize()
{
    preInitialize();
   // cModule *UE;
    //std::string str="stationary";
   // UE=deployUes(nodeX_,nodeY_,1,1,str,1,1,1);
