...
#include "corenetwork/deployer/LteDeployer2.h"
#include "world/radio/ChannelControl.h"
#include "world/radio/ChannelAccess.h"
#include "inet/mobility/static/StationaryMobility.h"


Define_Module(LteDeployer2);

// TODO: mettere parametro numEnbAntennas da passare a LteMcs::cwMapping (@par antennas)

LteDeployer2::LteDeployer2()
{
    //mobility=check_and_cast<StationaryMobility*>(
    getParentModule();
            //->getSubmodule("mobility");
    //mobility=check_and_cast<StationaryMobility*>(getSimulation()->getModuleByPath("mobility"));
  //  Coord temp;
   // temp=mobility->getCurrentPosition();
    numRelays_ = 0;
    relayDistance_ = 0;
   // nodeX_ = temp.x;
   // nodeY_ = temp.y;
...
void LteDeployer2::preInitialize()
{
...
//    }
    ...
    //deployUes()
    //deployUes(double centerX, double centerY, int Ue,
       // int totalNumOfUes, std::string mobType, int range, uint16_t masterId,
        //double speed)
}

void LteDeployer2::initialize()
{
    preInitialize();
    return;
}

int LteDeployer2::deployRelays(double startAngle, int i, int num, double *xPos,
    double *yPos)
{
...
cModule* LteDeployer2::deployUes(double centerX, double centerY, int Ue,
    int totalNumOfUes, std::string mobType, int range, uint16_t masterId,
    double speed)
{
    double x = 0;
...
void LteDeployer2::calculateNodePosition(double centerX, double centerY, int nTh,
...
uint16_t LteDeployer2::createRelayAt(double x, double y)
{
...
cModule* LteDeployer2::createUeAt(double x, double y, std::string mobType, /*std::string appType,*/
...
void LteDeployer2::attachMobilityModule(cModule *parentModule,
...
void LteDeployer2::deployRu(double nodeX, double nodeY, int numRu, int ruRange)
{
    if (numRu == 0)
        return;
    double x = 0;
...
void LteDeployer2::calculateMCSScale(double *mcsUl, double *mcsDl)
{
...
void LteDeployer2::updateMCSScale(double *mcs, double signalRe,
...
void LteDeployer2::createAntennaCwMap()
{
...
void LteDeployer2::detachUser(MacNodeId nodeId)
{
...
void LteDeployer2::attachUser(MacNodeId nodeId)
{
